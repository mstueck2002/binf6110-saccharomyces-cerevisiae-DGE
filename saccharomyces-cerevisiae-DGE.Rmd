...
list.files("fastq")
#Remove underscore and number following SRR accession ID
...
#Quantify samples against salmon index
...
#Set text IDs and gene IDs from reference fasta
tx_id <- sub(" .*", "", names(ref_fasta))
gene_id <- sub(".*locus_tag=([^]]+).*", "\\1", names(ref_fasta))

#Create txt2gene for later count data import
tx2gene <- data.frame(
  tx = tx_id,
  gene = gene_id,
...
#Mark genes as upregulated, downregulated, or not significant to colour them in ggplot. Axclude genes with under 2-fold change (log2FoldChange < 1)
...
#Volcano plot - log2foldchange against -log10(adjusted p value)
...
#Mark genes as upregulated, downregulated, or not significant to colour them in ggplot. Exclude genes with under 2-fold change (log2FoldChange < 1)
...
#Extract differential expression results - MATURE vs THIN
...
#Sanity check length of significant lrt genes                
length(sigLRT_genes)

#Variance stabilize significant genes
rld <- vst(dds)
rld_mat <- assay(rld)
...
#Retain only identifiers of interest
gene_map <- gene_map[c("protein_id","locus_tag","ORF")]
#Rename ORf as GENENAME
colnames(gene_map) <- c("protein_id","locus_tag","GENENAME")

#Map gene name ORF from blast to common yeast ORF
orf_map <- select(
    org.Sc.sgd.db,
...
#Merge clusters data frame with blast annotation data frame
cluster_orfs <- merge(
    gene_orf_clean,
    clusters_df,
    by = "locus_tag"
...
#Create new dataframe for annotations of thin vs early DEGs
mat_thin_genes <- rownames(mat_thin)

#Establish locus tags of matrix thin vs early DEGs
annot_thin <- as.data.frame(mat_thin) %>%
  mutate(locus_tag = mat_thin_genes) %>%
  left_join(gene_orf_clean, by = "locus_tag")

#If a gene symbol (GENENAME) is missing, fall back to using the locus tag.
annot_thin$GENENAME <- coalesce(annot_thin$GENENAME,
                                  annot_thin$locus_tag)

#Construct unique row names for heatmap labeling: 
##If no gene symbol exists, display only the locus tag. 
##If a gene symbol exists, display "locus_tag:GENENAME".
...
#Keep only expression values and columns necessary for heatmap
...
#Create new dataframe for annotations of mature vs early DEGs
mat_mature_genes <- rownames(mat_mature)

#Establish locus tags of matrix mature vs early DEGs
...
#Mature vs Thin Heatmap
#Create new dataframe for annotations of mature vs thin DEGs
